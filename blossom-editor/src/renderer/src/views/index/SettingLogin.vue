    ...
    <div style="width: 100%; text-align: center">
      ...
      <bl-row just="center">
        <div class="input-wrapper" style="width: 235px; margin-right: 30px">
          <input v-model="formLogin.username" type="text" class="form__input" placeholder="username" @input="handleServerUsername" @keyup.enter="login" />
          <div class="iconbl bl-user-line"></div>
        </div>
        <div class="input-wrapper" style="width: 235px">
          <input v-model="formLogin.password" type="password" class="form__input" placeholder="password" @keyup.enter="login" />
          <div class="iconbl bl-a-Securitypermissions-line"></div>
        </div>
      </bl-row>
      <!-- <button class="custom-btn btn-logout" style="margin-right: 30px" @click="logout"><span>退出</span></button> -->
      <button :class="['custom-btn btn-login', logingIn ? 'loging-in' : '']" @click="login"><span>登录</span></button>
      <!-- <bl-col height="80px" just="center">
        <div class="login-progress">
          <el-progress :text-inside="true" :stroke-width="20" :percentage="loginDesc.percentage" striped striped-flow :duration="30" :color="loginDesc.color">
      ...
      <!-- <bl-row v-if="configStore.viewStyle.isShowTryuseBtn" just="center">
        <span class="try-use" @click="showTryUse">我想试用!</span>
      </bl-row> -->
    </div>
  </div>

  <el-dialog v-model="isShowTryUse" draggable :align-center="true" :append-to-body="true" :destroy-on-close="true" :close-on-click-modal="true" width="550">
...
onMounted(() => {
  formLogin.value.serverUrl = 'http://47.96.77.127:9999'
...
const serverUrlIsInValid = computed(() => {
  const url = formLogin.value.serverUrl.toLocaleLowerCase()
  if (url.includes('/#/')) {
    return true
  }
  return false
...
.login-root {
  ...
  .input-wrapper {
    ...
    input {
      ...
      [class='dark'] & {
        box-shadow: inset 0.2rem 0.2rem 0.5rem #232323dd, inset -0.1rem -0.1rem 0.5rem #4e4e4e;
      }
  ...
  .btn-logout {
    ...
    box-shadow: inset 2px 2px 2px 0px #ffffff80, 7px 7px 20px 0px rgba(0, 0, 0, 0.1), 4px 4px 5px 0px rgba(0, 0, 0, 0.1);

    [class='dark'] & {
      box-shadow: inset 2px 2px 2px 0px #59595980, 7px 7px 20px 0px rgba(0, 0, 0, 0.1), 4px 4px 5px 0px rgba(0, 0, 0, 0.1);
    }
